...
def get_loader(split='train', batch_size=6, shuffle=True, num_workers=0, num_data=None, crop=True):  ####num worker 8....1
    """Get torch dataloader."""
    rgb_image_paths, lidar_image_paths, gt_image_paths, normal_image_paths = get_paths(split)##获得所有数据的路径信息
...
class depth_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # rgb1 = read_rgb1(self.rgb_image_paths[idx])
        ...
        # print(self.normal_image_paths[idx])
        # print(self.gt_image_paths[idx])
        # print(idx)
        surface_normal, mask_normal = read_normal(self.normal_image_paths[idx])


        # 随机裁剪图像
        height, width, channel = rgb.shape#################################################################333333333333333333333333333333333333
        ...
        if self.crop_or_not:
            ...
            # rgb1 = self._crop(rgb1, x_lefttop, y_lefttop, self.h, self.w)
            ...
            ####对RGB图片进行处理  重加载RGB 多增加一个返回值 作为语义分割图片
            # yuyi=test1(rgb)################################################################################
            # print(rgb.shape,"!3")
